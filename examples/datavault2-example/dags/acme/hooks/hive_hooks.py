...
class HiveCliHook(BaseHook):
 ...
 "-hiveconf", "hive.exec.dynamic.partition.mode=nonstrict"]
        ...
        return as_flattened_list(
            itertools.izip(
                ["-hiveconf"] * len(d), ["{}={}".format(k, v) for k, v in d.items()]
            )
        )
    ...
    def run_cli(self, hql, schema=None, verbose=True, hive_conf=None):
        ...
        with TemporaryDirectory(prefix="airflow_hiveop_") as tmp_dir:
            with NamedTemporaryFile(dir=tmp_dir, delete=False) as f:
                ...
                logging.info("{0}".format(os.path.exists(tmp_dir)))
                logging.info("{0}".format(os.path.isfile(f.name)))
                logging.info(
                    "Directory Permissions: {0}".format(
                        oct(os.stat(tmp_dir).st_mode)[-3:]
                    )
                )
                logging.info(
                    "File Permissions: {0}".format(oct(os.stat(f.name).st_mode)[-3:])
                )
                ...
                if self.mapred_job_name:
                    hive_conf_params.extend(
                        ["-hiveconf", "mapred.job.name={}".format(self.mapred_job_name)]
                    )
    ...
    def transfer_data_file(self, filepath, verbose=True):
        ...
        sp = subprocess.Popen(
            hdfs_cmd, stdout=subprocess.PIPE, stderr=subprocess.STDOUT
        )
        self.sp = sp
        stdout = ""
        while True:
    ...
    def load_df(
        ...
        **kwargs
        ...
        def _infer_field_types_from_df(df):
            ...
            return dict(
                (col, DTYPE_KIND_HIVE_TYPE[dtype.kind])
...
class HiveMetastoreHook(BaseHook):
    ...
    def get_metastore_client(self):
        ...
        if (
            configuration.get("core", "security") == "kerberos"
    ...
    def check_for_named_partition(self, schema, table, partition_name):
        ...
        self.metastore._oprot.trans.open()
        try:
            self.metastore.get_partition_by_name(schema, table, partition_name)
            return True
    ...
    def max_partition(self, schema, table_name, field=None, filter=None):
        ...
        if not parts:
            ...
            raise AirflowException(
                "Please specify the field you want the max " "value for"
            )
