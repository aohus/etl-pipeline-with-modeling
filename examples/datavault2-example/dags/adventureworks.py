...
staging_done = DummyOperator(task_id="staging_done", dag=dag)
hubs_done = DummyOperator(task_id="hubs_done", dag=dag)
links_done = DummyOperator(task_id="links_done", dag=dag)
sats_done = DummyOperator(task_id="sats_done", dag=dag)
enddating_done = DummyOperator(task_id="enddating_done", dag=dag)
...
def remove_microseconds(ts_nodash):
    return ts_nodash.split(".")[0]
...
def create_operator_array(hql, hive_table):
    ...
    step3 = HiveOperator(
        hql="INSERT OVERWRITE TABLE dv_raw.{0} SELECT * FROM dv_temp.{0}_temp".format(
            hive_table
        ),
...
create_staging_operator(
    sql="staging/salesorderdetail.sql", hive_table="salesorderdetail"
)
create_staging_operator(
    sql="staging/salesorderheader.sql", hive_table="salesorderheader"
)
create_staging_operator(
...
# links
create_link_operator(
    "loading/link_address_stateprovince.hql", "link_address_stateprovince"
)
...
create_link_operator(
    "loading/link_salesorder_creditcard.hql", "link_salesorder_creditcard"
)
create_link_operator(
    "loading/link_salesorder_currencyrate.hql", "link_salesorder_currencyrate"
)
...
create_link_operator(
    "loading/link_salesorder_shipmethod.hql", "link_salesorder_shipmethod"
)
